...
export const AuthForm = () => {
    ...
    const login = async () => {
        try {
            let res = await axios.post('/auth/login', { email: email.value, password: password.value})
    ...
    return (
        <FormWrap>
            ...
            />
            <Button type={"button"} disabled={!password.value.length || !email.value.length} onClick={submit}>Sign up</Button>
        </FormWrap>
    )
}